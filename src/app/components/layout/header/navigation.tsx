      ...
      {["MUSIK", "PARTNERE", "FRIVILLIG", "GALLERI", "PRAKTISK", "KØB BILLETTER"].map((text, index) => (
        <Link href={`/${text.toLowerCase().replace(" ", "-")}`} key={index} className="flex-1">
          <motion.div
            ...
            ${index === 5 ? "bg-gradient-to-br from-[#2D2D2D] to-[#1D1D1D] text-white hover:from-[#1D1D1D] hover:to-[#0D0D0D]" : "bg-gradient-to-br from-[#E5E5E5] to-[#D9D9D9] hover:from-[#D5D5D5] hover:to-[#C9C9C9]"}
            transition-all duration-3500 hover:shadow-inner`}
          >
            {text}
          </motion.div>
        </Link>
