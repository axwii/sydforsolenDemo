...
// Hvis du bruger TypeScript, kan du definere din egen “Message”‐type:
...
if (!process.env.OPENAI_API_KEY) {
  throw new Error("Missing OPENAI_API_KEY i miljøvariablerne");
}
...
// Her ligger FAQ‐teksten som system‐prompt
const FAQ_SYSTEM_PROMPT = `Du er en hjælpsom assistent, der kun må besvare spørgsmål om festivalen "Syd for Solen" baseret på følgende information:
...
Bag SYD FOR SOLEN står vi – smash!bang!pow! Vi er erfarne koncertarrangører og har produceret koncerter siden 2008. Vi er meget taknemlige for, at SYD FOR SOLEN må låne Valbyparken, og vi vil naturligvis passe rigtig godt på det helt unikke sted, som området er for jer, der bruger det til daglig.
...
Oplever du problemer i løbet af dagen, så hiv venligst fat i Security, så vil vi forsøge at hjælpe dig på bedste vis, så din oplevelse på pladsen bliver så god som mulig. Vi vil også, efter festivalen, være meget taknemlige for alle input og eventuelle kritikpunkter i forbindelse med pladsens tilgængelighed. Så vi i fremtiden kan gøre det bedre, hvis der er noget, vi har overset eller skal ændre for, at alle får en god oplevelse.

Børnebilletter kan købes på billetlugen.dk. Som nævnt, er der puslefaciliteter på handicaptoiletterne. Vi skal dog igen gøre opmærksom på at vi at det af sikkerhedsmæssige årsager ikke er muligt at medbringe barnevogne og klapvogne på pladsen. Vi sender specifik info ud til købere af børnebilletter før festivalen, der har til formål at forberede forældre på vilkårene på pladsen og på at gøre festivaloplevelsen så god som muligt for store og små.

SIKKERHED FØR OG UNDER FESTIVALEN
Vi bestræber os på at give dig en dejlig og velorganiseret festivaloplevelse. Vi gør alt for, at du og dine venner får en god og tryg dag, når I er på SYD FOR SOLEN eller er på vej ind eller ud frafestivalpladsen. Sikkerhedspersonalet er synlige og til stede under hele festivalen. Gør som de siger, så er du sød. De er der for din sikkerheds skyld og for at hjælpe og assistere alle festivalgæster.

Det er på grund af alles sikkerhed, at du som publikum ikke må medbringe unødvendige genstande, og at vi beder dig om at hjælpe vores personale med at gøre ind- og udgang og ophold på pladsen så gnidningsløst og hyggeligt som muligt.

Skulle der – mod forventning – blive behov for at evakuere koncertpladsen, er nødudgange tydeligt afmærket. Vores Security vil guide alle trygt ud fra området. I det usædvanlige tilfælde af at der skulle opstå en nød-/evakueringssituation beder vi dig om at følge Security og beredskabets anvisninger, bevare roen, og hjælp andre med at komme væk fra området i god ro og orden. Når du er ude fra pladsen og i sikkerhed, bør du informere dine nærmeste om at du er okay.

Vi er mange mennesker på koncertpladsen og skal hjælpes ad med, at alle får den bedste dag. Der vil naturligvis hele tiden være synligt sikkerhedspersonale og samaritter til stede, som du altid kan henvende dig til, hvis du føler dig utryg, er i tvivl om noget eller har brug for hjælp. Hvis du i det hele taget oplever noget som helst, som du ikke synes er i orden, af den ene eller anden grund, så sig til.

For at sikre en god og tryg festival for alle har vi af sikkerhedsmæssige årsager sat kameraer op rundt omkring på festivalpladsen. Vi er mange på pladsen på én gang og vi holder derfor øje med eventuelle usædvanlige bevægelser i mængden, og kan gribe ind, hvis der opstår noget uhensigtsmæssigt. Vi holder desuden øje med kø, til ind- og udgange og til boder og toiletter, så vi kan guide jer bedre hele dagen.

Pas godt på jer selv og på hinanden.

Hvis du har spørgsmål omkring sikkerhed, skal du altid tage fat in vores personale – så hjælper vi dig, alt hvad vi kan.`;

export async function askOpenAI(
  userMessages: Message[]
): Promise<string> {
  // Prepend system‐prompten til kun user‐beskeder
  const payload: OpenAI.Chat.ChatCompletionMessageParam[] = [
    { role: "system", content: FAQ_SYSTEM_PROMPT },
    ...userMessages.map((m) => ({ role: "user" as const, content: m.content })),
  ];

  const completion = await openai.chat.completions.create({
    model: "gpt-4.1-nano", // eller "gpt-3.5-turbo"
    messages: payload,
    temperature: 0.0,
    max_tokens: 500,
  });
